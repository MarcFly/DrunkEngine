...
update_status ModuleRenderer3D::PreUpdate(float dt)
{
	...
	// light 0 on cam pos
	lights[0].SetPos(App->camera->Position.getX(), App->camera->Position.getY(), App->camera->Position.getZ());
...
void ModuleRenderer3D::OnResize(int width, int height)
{
	...
	glMatrixMode(GL_PROJECTION);
	glLoadIdentity();
	//ProjectionMatrix. (60.0f, (float)width / (float)height, 0.125f, 512.0f);
